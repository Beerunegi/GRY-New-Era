...
import { ArrowRight, BarChart3, Target, Activity } from "lucide-react";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary font-bold text-sm mb-6 uppercase tracking-wider backdrop-blur-sm border border-primary/20">
              <BarChart3 className="w-3.5 h-3.5" /> Stop Guessing. Start Scaling.
            </div>
            
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tight mb-6 leading-[1.1]">
              Data-Driven <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-primary/80 to-secondary selection:bg-primary/20">Decision Making</span>
            </h1>
            
            <p className="text-lg text-muted-foreground leading-relaxed mb-8 max-w-lg">
              Unlock the full potential of your marketing budget. We implement advanced tracking architecture to show you exactly which campaigns drive revenue and where you're wasting ad spend.
            </p>

            <div className="flex flex-col sm:flex-row gap-4 mb-10">
              <Button size="lg" className="h-14 px-10 text-lg shadow-xl shadow-primary/20 font-bold group">
                Get Tracking Setup <ArrowRight className="w-5 h-5 ml-2 group-hover:translate-x-1 transition-transform" />
              </Button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-8 pt-6 border-t border-border/50 max-w-sm">
              <div className="flex items-center gap-3">
                ...
                <div className="text-base font-bold opacity-80 uppercase tracking-widest text-[10px]">100% Attribution</div>
              </div>
              <div className="flex items-center gap-3">
                <div className="p-2 rounded-xl bg-secondary/5 border border-secondary/10">
                  <Activity className="w-5 h-5 text-secondary" />
                </div>
                <div className="text-base font-bold opacity-80 uppercase tracking-widest text-[10px]">Real-Time Data</div>
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="absolute -bottom-6 -right-6 bg-card/80 backdrop-blur-xl p-5 rounded-3xl border border-white/20 shadow-2xl hidden md:block">
              <div className="text-2xl font-black text-primary mb-1 tracking-tight">GA4 Ready</div>
              <div className="text-[9px] text-muted-foreground font-black uppercase tracking-[0.2em]">Certified Implementation Specialists</div>
            </div>
          </motion.div>
        </div>
      </div>
