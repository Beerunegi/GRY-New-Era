...
import { ArrowRight, PenTool, BarChart2, MessageSquare } from "lucide-react";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary font-bold text-sm mb-6 uppercase tracking-wider backdrop-blur-sm border border-primary/20">
              <PenTool className="w-3.5 h-3.5" /> High-Authority Content
            </div>
            
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tight mb-6 leading-[1.1]">
              Fuel Your Growth with <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-primary/80 to-secondary selection:bg-primary/20">Strategic Content</span>
            </h1>
            
            <p className="text-lg text-muted-foreground leading-relaxed mb-8 max-w-lg">
              We create compelling, SEO-driven content that captures attention, builds authority, and converts visitors into loyal customers.
            </p>

            <div className="flex flex-col sm:flex-row gap-4 mb-10">
              <Button size="lg" className="h-14 px-10 text-lg shadow-xl shadow-primary/20 font-bold group">
                Get Content Strategy <ArrowRight className="w-5 h-5 ml-2 group-hover:translate-x-1 transition-transform" />
              </Button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-8 pt-6 border-t border-border/50 max-w-sm">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="text-base font-bold opacity-80 uppercase tracking-widest text-[10px]">Brand Voices</div>
              </div>
            </div>
          </motion.div>
          ...
          >
            <div className="relative aspect-[4/3] rounded-[2.5rem] overflow-hidden shadow-2xl border border-border/50 group bg-muted">
              <Image 
                ...
                alt="Content Strategy and Branding" 
                fill 
            ...
            <div className="absolute -bottom-6 -right-6 bg-card/80 backdrop-blur-xl p-5 rounded-3xl border border-white/20 shadow-2xl hidden md:block">
              <div className="text-2xl font-black text-primary mb-1 tracking-tight">AUTHORITY</div>
              <div className="text-[9px] text-muted-foreground font-black uppercase tracking-[0.2em]">Verified Results</div>
            </div>
          </motion.div>
        </div>
      </div>
